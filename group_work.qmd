...
data_2['combined_value'] = data_2['occupancy_rate'] + 2*data_2['reviews_per_day']
...
# 假设 data_2 已经包含了经纬度信息和 'star_rating' 列
...
#gdf_boroughs_new.plot(ax=ax, color='lightgrey')

# 绘制点，使用评级信息来改变颜色
#gdf_points_new.plot(ax=ax, column='star_rating', cmap='viridis', legend=True, markersize=50)

# 添加图例和标题
#plt.legend(title='Star Rating')
#plt.title('Star Ratings in London')
...
# 绘制点，根据 'satisfaction_level' 改变颜色
#gdf_quality.plot(ax=ax, column='satisfaction_level', cmap='viridis', legend=True, markersize=50)

# 添加图例和标题
#plt.legend(title='Satisfaction Level')
#plt.title('Satisfaction Levels in London')
...
```{python}
# 创建包含两个子图的画布，这里的 figsize 需要根据实际情况调整
...
scatter1 = gdf_points_new.plot(ax=axs[0], column='star_rating', cmap='viridis', markersize=1)

# 第二个子图：品质等级
gdf_boroughs_new.plot(ax=axs[1], color='white', edgecolor='lightgrey')
scatter2 = gdf_quality.plot(ax=axs[1], column='satisfaction_level', cmap='viridis', markersize=1)

# 添加图例，位于图的右侧
fig.colorbar(scatter1.get_children()[0], ax=axs, orientation='vertical', fraction=0.02, pad=0.02)

# 设置标题
axs[0].set_title('Popular Levels in London')
axs[1].set_title('Quality Levels in London')
...
```{python}
def textual_alazying(dataDescription):
    # fill in the Na values
    corpus = dataDescription.description.fillna(' ').values
    fcounts = dict()
    
    #fitting the cvectorize with corpus
    cvectorizer = CountVectorizer(ngram_range=(1,2))
    cvectorizer.fit(corpus)
    #Convert text data into a matrix
    cvtcorpus = cvectorizer.transform(corpus)
    
    cvdf = pd.DataFrame(data=cvtcorpus.toarray(),columns=cvectorizer.get_feature_names_out())
    #Filter Low-Frequency Words
    filter_terms = cvdf.sum(axis=0) >= cvdf.shape[0] * 0.01
...
```{python}
description = []
# select the houses which is 5 in star_rating
...
# select the houses which is 4 in star_rating
...
# select the houses which is 3 in star_rating
...
# select the houses which is 2 in star_rating
...
# select the houses which is 1 in star_rating
...
```{python}
cvdf5 = textual_alazying(analyzeRank5)
cvdf4 = textual_alazying(analyzeRank4)
cvdf3 = textual_alazying(analyzeRank3)
cvdf2 = textual_alazying(analyzeRank2)
cvdf1 = textual_alazying(analyzeRank1)
```

```{python}
cvdf5.sum().sort_values(ascending=False).head(10)
```

```{python}
cvdf4.sum().sort_values(ascending=False).head(10)
```

```{python}
cvdf3.sum().sort_values(ascending=False).head(10)
```

```{python}
cvdf2.sum().sort_values(ascending=False).head(10)
```

```{python}
cvdf1.sum().sort_values(ascending=False).head(10)
```

```{python}
# Deletion of a large number of co-existing words : space, london,
cvdf5.sum().sort_values(ascending=False).iloc[9:20]
```

```{python}
cvdf1.sum().sort_values(ascending=False).iloc[9:20]
```

```{python}
comparison = cvdf3.sum().sort_values(ascending=False).compare(cvdf4.sum().sort_values(ascending=False))
print(comparison)
```
